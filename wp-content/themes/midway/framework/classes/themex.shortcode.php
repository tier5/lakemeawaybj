...
class ThemexShortcode {
	...
	public static function initScripts() {
		...
		$out.='themexTitle="'.__('Insert Shortcode', 'midway').'";';
	...
	public static function renderSettings() {
		...
		if(isset(self::$data['clone'])) {
			...
			$out.='<a href="#" class="themex-button themex-remove-button themex-trigger" data-element="'.$ID.'" title="'.__('Remove', 'midway').'"></a>';
			$out.='<a href="#" class="themex-button themex-clone-button themex-trigger" data-element="'.$ID.'" data-value="'.$ID.'" title="'.__('Add', 'midway').'"></a>';
		...
		$out.='<tr><th></th><td><div class="themex-option themex-submit"><input type="submit" class="themex-button" value="'.__('Insert Shortcode','midway').'" /></div></td></tr>';
		$out.='</tbody></table>';
	...
	public static function filterShortcodes($content) {
		...
		$filtered=preg_replace("/(<p>)?\[($shortcodes)(\s[^\]]+)?\](<\/p>|<br \/>)?/", "[$2$3]", $content);
		$filtered=preg_replace("/(<p>)?\[\/($shortcodes)](<\/p>|<br \/>)?/", "[/$2]", $filtered);
